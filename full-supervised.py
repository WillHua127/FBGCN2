...
print(device,checkpt_file)

def train_step(model,optimizer,features,labels,adj,adj_high,idx_train):
    model.train()
    optimizer.zero_grad()
...
def validate_step(model,features,labels,adj,adj_high,idx_val):
    model.eval()
    with torch.no_grad():
        output = model(features,adj,adj_high)
...
def train(datastr,splitstr):
    adj, adj_high, features, labels, idx_train, idx_val, idx_test, num_features, num_labels = full_load_data(datastr,splitstr)
    features = features.to(device)
    adj, adj_high = adj.to(device), adj_high.to(device)
    ...
    for epoch in range(args.epochs):
        loss_tra,acc_tra = train_step(model,optimizer,features,labels,adj, adj_high, idx_train)
        loss_val,acc_val = validate_step(model,features,labels,adj, adj_high,idx_val)
        if(epoch+1)%1 == 0: 
        ...
        if bad_counter == args.patience:
            break
    acc = test_step(model,features,labels,adj, adj_high, idx_test)[1]
